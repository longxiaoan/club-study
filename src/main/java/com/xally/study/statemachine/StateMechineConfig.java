...
@Configuration
@EnableStateMachine
public class StateMechineConfig extends EnumStateMachineConfigurerAdapter<States, Events> {
	...
	@Override public void configure(StateMachineTransitionConfigurer<States, Events> transitions) throws Exception {
		transitions.withExternal().source(States.UNPAID).target(States.WAITING_FOR_RECEIVE).event(Events.PAY).and().withExternal().source(States.WAITING_FOR_RECEIVE).target(States.DONE)
